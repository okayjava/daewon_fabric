...
// });
    	...
    	await stub.putState(deliveryInfo.dv_id, Buffer.from(JSON.stringify(deliveryInfo)));
        let indexName = 'dv_id';
        let indexKey = await stub.createCompositeKey(indexName, [prod.dv_id]);
        console.info(indexKey);
        console.info('- end put delivery information');
  }

  // Regist product info
  async registProd(stub, args, thisClass) {
        let prod = {};
        prod.dstrb_id = '';
        prod.lat = '';
        prod.lng = '';
...
//     console.log(ele.TxId);
// });
        prod.dstrb_id = json.dstrb_id;
        prod.lat = json.lat;
        prod.lng = json.lng;
        prod.full_addr = json.full_addr;
        prod.prv_nm = json.prv_nm;
        prod.city_nm = json.city_nm;
        prod.dstr_nm = json.dstr_nm;
        prod.dstrb_typ_cd = json.dstrb_typ_cd;
        prod.dstrb_typ_nm = json.dstrb_typ_nm;
        prod.dv_id = json.dv_id;
        prod.corp_id = json.corp_id;
        prod.corp_nm = json.corp_nm;
        prod.seq = json.seq;
        prod.corp_user_id = json.corp_user_id;
        prod.corp_user_nm = json.corp_user_nm;
        prod.prod_nm = json.prod_nm;
        prod.rgst_dtm = json.rgst_dtm;

    	await stub.putState(prod.seq, Buffer.from(JSON.stringify(prod)));
        let indexName = 'seq~dv_id';
        let indexKey = await stub.createCompositeKey(indexName, [prod.seq, prod.dv_id]);
        console.info(indexKey);
        console.info('- end put product');
  }
  // ===============================================
  ...
  async registProd(stub, args, thisClass) {
    ...
    async queryProdSimple(stub, args, thisClass) {
  ...
  async historyProd(stub, args, thisClass) {
    ...
    let prodSeq = args[0];
    console.info('- start historyProd: %s\n', prodSeq);

    let resultsIterator = await stub.getHistoryForKey(prodSeq);
