...
// ==== Invoke marbles ====
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["initMarble","marble1","blue","35","tom"]}'
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["initMarble","marble2","red","50","tom"]}'
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["initMarble","marble3","blue","70","tom"]}'
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["transferMarble","marble2","jerry"]}'
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["transferMarblesBasedOnColor","blue","jerry"]}'
// peer chaincode invoke -C mychannel -n marbles -c '{"Args":["delete","marble1"]}'

// ==== Query marbles ====
// peer chaincode query -C mychannel -n marbles -c '{"Args":["readMarble","marble1"]}'
// peer chaincode query -C mychannel -n marbles -c '{"Args":["getMarblesByRange","marble1","marble3"]}'
// peer chaincode query -C mychannel -n marbles -c '{"Args":["getHistoryForMarble","marble1"]}'
// peer chaincode query -C mychannel -n marbles -c '{"Args":["getMarblesByRangeWithPagination","marble1","marble3","3",""]}'

// Rich Query (Only supported if CouchDB is used as state database):
// peer chaincode query -C mychannel -n marbles -c '{"Args":["queryMarblesByOwner","tom"]}'
// peer chaincode query -C mychannel -n marbles -c '{"Args":["queryMarbles","{\"selector\":{\"owner\":\"tom\"}}"]}'

// Rich Query with Pagination (Only supported if CouchDB is used as state database):
// peer chaincode query -C mychannel -n marbles -c '{"Args":["queryMarblesWithPagination","{\"selector\":{\"owner\":\"tom\"}}","3",""]}'
...
let Chaincode = class {
  ...
  // regist delivery infomation
  async registDelivery(stub, args, thisClass) {
        let deliveryInfo = {};
        deliveryInfo.dv_id = '';
        deliveryInfo.dv_dtm = '';
        deliveryInfo.corp_id = '';
        deliveryInfo.corp_nm = '';
        deliveryInfo.lat = '';
        deliveryInfo.lng = '';
        deliveryInfo.corp_user_id = '';
        deliveryInfo.corp_user_nm = '';

    	let dv_id = args[0];
        let base64_data = args[1];
	console.log(base64_data);
        var t = Buffer.from(base64_data, 'base64').toString();
	//var t = Buffer.from(base64_data, 'base64').toString('ascii').replace('\\','');
...
// });
        deliveryInfo.dv_id = json.dv_id;
        deliveryInfo.dv_dtm = json.dv_dtm;
        deliveryInfo.corp_id = json.corp_id;
        deliveryInfo.corp_nm = json.corp_nm;
        deliveryInfo.lat = json.lat;
        deliveryInfo.lng = json.lng;
        deliveryInfo.corp_user_id = json.corp_user_id;
        deliveryInfo.corp_user_nm = json.corp_user_nm;

    console.info('before put State');
    	await stub.putState(dv_id, Buffer.from(JSON.stringify(deliveryInfo)));
    console.info('after put State');
        let indexName = 'dv_id';
        let indexKey = await stub.createCompositeKey(indexName, [deliveryInfo.dv_id]);
        console.info(indexKey);
        console.info('- end put delivery information');
  }

  async historyDelivery(stub, args, thisClass) {
    ...
    let dv_id = args[0];
    console.info('- start Delivery history: %s\n', dv_id);

    let resultsIterator = await stub.getHistoryForKey(dv_id);
    let method = thisClass['getProdAllResults'];
  ...
  // Only available on state databases that support rich query (e.g. CouchDB)
  // =========================================================================================
  async queryProd(stub, args, thisClass) {
    //   0
    // 'queryString'
    if (args.length < 1) {
      throw new Error('Incorrect number of arguments. Expecting queryString');
    ...
    let method = thisClass['getProdQueryResultForQueryString'];
    let queryResults = await method(stub, queryString, thisClass);
    return queryResults;
  }

  async getProdAllResults(iterator, isHistory) {
    let allResults = [];
    while (true) {
      let res = await iterator.next();
  ...
  // Result set is built and returned as a byte array containing the JSON results.
  // =========================================================================================
  async getProdQueryResultForQueryString(stub, queryString, thisClass) {
    ...
    async getProdHistorySimple(stub, queryString, thisClass) {
        console.info('============= START : getHistory ===========');
        let carNumber = arg[1];
        const carAsBytes = await stub.getState(carNumber); // get the car from chaincode state
        if (!carAsBytes || carAsBytes.length === 0) {
            throw new Error(`${carNumber} does not exist`);
        }

        const iterator = await stub.GetHistoryForKey(carNumber);
        const allResults = [];
        while (true) {
